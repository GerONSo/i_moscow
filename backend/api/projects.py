...
def create_project(cookie):
    ...
    project = Project(master_id=account.id,
                      chat_id=ChatRepository(owner_id=account.id, chat_type=ChatTypes.GROUP).chat.id,
                      **request.json)
    ...
    mydb.execute("update accounts set master_project_ids = %s, chat_ids = %s where id = %s",
                 (json.dumps(account.master_project_ids), json.dumps(account.chat_ids), account.id))
    myconnect.commit()
...
def join_to_project(cookie):
    ...
    chat = common.actions.get_person_chat_by_members(account.id, project.master_id)
    chat = ChatRepository(id=chat.id)
    chat.set_invite_status(project.master_id, InviteStatus.ACCEPTED)
    return project.to_primitive()


@app.route("/not_join_to_project/<cookie>", methods=["POST"])
def not_join_to_project(cookie):
    user_id = common.actions.make_cookie_authorize(cookie)
    project_id = request.json["project_id"]
    if user_id is None:
        return {}, 401
    ...
    chat = common.actions.get_person_chat_by_members(account.id, project.master_id)
    chat = ChatRepository(id=chat.id)
    chat.set_invite_status(project.master_id, InviteStatus.DECLINED)
    return project.to_primitive()
...
def accept_to_project(cookie):
    ...
    chat = ChatRepository(project.chat_id)
    chat.set_invite_status(slave.id, InviteStatus.ACCEPTED)
    return project.to_primitive()


@app.route("/not_accept_to_project/<cookie>", methods=["POST"])
def not_accept_to_project(cookie):
    user_id = common.actions.make_cookie_authorize(cookie)
    project_id, slave_id = request.json["project_id"], request.json["slave_id"]
    if user_id is None:
        return {}, 401
    ...
    chat = ChatRepository(project.chat_id)
    chat.set_invite_status(slave.id, InviteStatus.DECLINED)
    return project.to_primitive()
...
def send_project_request(cookie):
    ...
    myconnect.commit()
    chat.send_message(person_id=account.id, message_type=MessageTypes.INVITE, metadata={"project_id": project.id})
    return Response(json.dumps(account.chat_ids), mimetype='application/json')


@app.route("/send_slave_request/<cookie>", methods=["POST"])
def send_slave_request(cookie):
    user_id = common.actions.make_cookie_authorize(cookie)
    slave_id, project_id = request.json["slave_id"], request.json["project_id"]
    if user_id is None:
        return {}, 401
    ...
    slave = common.actions.get_account_by_id(slave_id)
    project = common.actions.get_project_by_id(project_id)
    if slave is None or project is None:
        return {}, 400
    if slave.id in project.slaves_id or slave.id == project.master_id or project.master_id != account.id:
        return {}, 400

    chat = common.actions.get_person_chat_by_members(account.id, slave.id)
    created_flag = not chat

    chat = ChatRepository(owner_id=account.id, chat_type=ChatTypes.PERSON) if chat is None else ChatRepository(chat.id)
    chat.add_member(person_id=account.id, member_id=slave.id, member_role=ChatRoles.DEFAULT)

    if created_flag:
        account.chat_ids.append(chat.chat.id)
        slave.chat_ids.append(chat.chat.id)
        mydb.execute("update accounts set chat_ids = %s where id = %s",
                     (json.dumps(account.chat_ids), account.id))
        mydb.execute("update accounts set chat_ids = %s where id = %s",
                     (json.dumps(slave.chat_ids), slave.id))
    myconnect.commit()
